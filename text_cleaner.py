...
class TextImg:
    ...
    def refine_text(self):
        ...
        invert = cv2.bitwise_not(self.img).astype(np.uint16)
        self.img = cv2.filter2D(invert, cv2.CV_64F, kernel, borderType=cv2.BORDER_REPLICATE)
        M = cv2.getStructuringElement(cv2.MORPH_RECT, (3,3))
        self.img = cv2.dilate(self.img, M)
    ...
    def skew_correction(self, angle=None):
        ...
        h, w = self.img.shape[:2]
        self.img = cv2.warpAffine(self.img, M, (w, h), flags=cv2.INTER_LANCZOS4, borderMode=cv2.BORDER_REPLICATE)
...
if __name__ == "__main__":
    ...
    text.refine_text()
    cv2.imshow("Text Refining", text.img)
    cv2.waitKey(0)
